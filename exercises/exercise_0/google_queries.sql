...
SELECT keyword, COUNT(*) AS searches FROM GOOGLE_KEYWORDS GROUP BY keyword ORDER BY searches DESC LIMIT 10;

-- Unique keywords
SELECT COUNT(DISTINCT keyword) FROM GOOGLE_KEYWORDS; -- 7263686

SELECT DISTINCT(platform) FROM GOOGLE_KEYWORDS; -- 1 (desktop)
...
FROM GOOGLE_KEYWORDS
GROUP BY platform
ORDER BY users_per_platform DESC; -- 35046855
...
FROM GOOGLE_KEYWORDS
WHERE LOWER(keyword) LIKE '%spotify%'
GROUP BY country
ORDER BY spotify_searches DESC
LIMIT 10;